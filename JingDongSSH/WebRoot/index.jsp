...
<DL>
  ...
  href="/JingDong/shoppingCar.jsp"><img src="<%=path%>/image/gouwuchejiezhang.jpg"/></A> 
  </DT>
  <DD>
  <DIV class=prompt>
...
<div id="content">
	...
	<c:forEach var="list" items="${list}" varStatus="index">
			<c:if test="${index.index % 5 != 0}">
					<div id="goods">
					<dl>
						<dt><a href="<%=path%>/goods/GoodsInfoAction?id=${list.id}"><img src="<%=path%>/${list.imgURL}"/></a></dt>
						<dd id="dd_1"><a href="<%=path%>/goods/GoodsInfoAction?id=${list.id}">
							<c:choose>
						...
						<dd id="dd_2"><img src="<%=path%>/image/qian.jpg"/>${list.price}</dd>
						<dd id="dd_3"><a href="###">已有${list.evaluate}多人评价了</a> <img src="<%=path%>/image/zhijiang.jpg"/> <img src="<%=path%>/image/zengpin.jpg"/> <img src="<%=path%>/image/fanjuan.jpg"/> </dd>
						<dd id="dd_4"><a href="<%=path%>/goods/BuyGoodsAction?id=${list.id}"><img src="<%=path%>/image/goumai.jpg"/></a>&nbsp;<a href=""><img src="<%=path%>/image/guanzhu1.jpg"/></a>&nbsp;<a href=""><img src="<%=path%>/image/duibi.jpg"/></a></dd>
					</dl>
				</div>
			</c:if>
	</c:forEach>
